...
for stock in set_tscode:
    ...
    pd_judge = pd_temp[(pd_temp['加权平均净资产收益率(%)'] < 15) | (pd_temp['销售商品提供劳务收到的现金/营业收入'] < 1) | (pd_temp['经营活动产生的现金流量净额/经营活动净收益'] < 1) |
               (pd_temp['扣非净利率(%)'] < 15) | (pd_temp['商誉净资产比'] > 0.5) | (pd_temp['存货净资产比'] > 0.5) | (pd_temp['应收账款净资产比'] > 0.5) |
               (pd_temp['流动比率'] < 2) | (pd_temp['速动比率'] < 1) | (pd_temp['营业总收入同比增长率(%)'] < 15) | (pd_temp['归属母公司股东的净利润-扣除非经常损益同比增长率(%)'] < 15)]
    if len(pd_judge):
        continue
    else:
        select = select.append(pd_temp)

select.to_csv('D:\\股票分析\\财务指标选股（每期都满足）_2017至2019年.csv')
